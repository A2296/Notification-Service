...
function notificationRow(item) {
  const channel = String(item.channel || 'in-app').toLowerCase();
  const status = String(item.status || 'pending').toLowerCase();
  const channelIcon = channelIcons[channel] || channelIcons['in-app'];

  return `<tr>
    <td>
      <strong>${escapeHtml(item.subject || item.title || 'Notification')}</strong>
      <span class="notification-id">${escapeHtml(item.id || item._id || '—')}</span>
    </td>
    <td>
      <span class="channel">${channelIcon} ${escapeHtml(channel)}</span>
    </td>
    <td>${escapeHtml(item.recipient || item.to || '—')}</td>
    <td>
      <span class="status ${escapeHtml(status)}">${escapeHtml(status)}</span>
    </td>
    <td>${escapeHtml(formatDate(item.createdAt || item.created_at))}</td>
  </tr>`;
}
...
function formatDate(value) {
  ...
  if (typeof value === 'string' && value.endsWith('ago')) {
    return value;
  }
...
function renderActivity() {
  ...
  const wantedStatus = el('#statusFilter').value.toLowerCase();

  const items = notifications.filter((item) => {
    const itemStatus = String(item.status || '').toLowerCase();

    return (
      (wantedStatus === 'all' || itemStatus === wantedStatus) &&
      JSON.stringify(item).toLowerCase().includes(query)
    );
  });

  el('#activityRows').innerHTML = items.map(notificationRow).join('');
  el('#recentActivity').innerHTML = notifications
    .slice(0, 5)
    .map(notificationRow)
    .join('');

  el('#emptyActivity').classList.toggle(
    'hidden',
    items.length > 0
  );
}
...
function updateSummary() {
  const counts = notifications.reduce((all, item) => {
  ...
  el('#deliveredCount').textContent =
    counts.delivered || counts.sent || 0;
...
el('#activitySearch').addEventListener(
  'input',
  renderActivity
);
...
el('#refreshActivity').addEventListener(
  'click',
  loadNotifications
);

el('#notificationForm').addEventListener(
  'submit',
  async (event) => {
    event.preventDefault();
    ...
    try {
      ...
      notifications.unshift({
        ...
        subject:
          ...
          payload.subject,
        recipient:
          createdNotification?.recipient ||
          payload.recipient,
        channel:
          createdNotification?.channel ||
          payload.channel,
        status:
      ...
      if (emailChannel) {
        emailChannel.checked = true;
      }

      message.textContent = API.baseUrl
        ? 'Notification submitted successfully.'
        : 'Demo notification added. Connect the backend to send it.';
...
el('#settingsButton').addEventListener('click', () => {
  el('#apiBaseUrl').value = API.baseUrl;
  el('#accessToken').value = API.token;

  dialog.showModal();
});

el('#settingsForm').addEventListener(
  'submit',
  (event) => {
    ...
    const live = Boolean(API.baseUrl);

    el('#connectionStatus').innerHTML =
      `<span></span> ${
        live ? 'API connected' : 'Demo mode'
      }`;

    el('#connectionStatus').classList.toggle(
      'live',
      live
    );
    ...
    showToast(
      live
        ? 'Connection saved. Loading API data…'
        : 'Demo mode enabled.'
    );
...
el('#authDialog .dialog-close').addEventListener(
  'click',
  () => {
    authDialog.close();
  }
...
function updateAuthForm() {
  ...
  el('.register-only').classList.toggle(
    'hidden',
    !registrationMode
  );

  el('#businessName').required =
    registrationMode;

  el('#authSubmit').innerHTML =
    `${registrationMode ? 'Create account' : 'Sign in'} <span>→</span>`;

  el('#toggleAuth').textContent =
    registrationMode
  ...
  async (event) => {
    ...
    if (!API.baseUrl) {
      authMessage.textContent =
        'Add your backend URL in Connection settings before signing in.';
      return;
    }
...
el('#generateKey').addEventListener(
  'click',
  async () => {
    if (!API.baseUrl) {
      el('#apiKeyDisplay').textContent =
      ...
      showToast('Demo test key generated.');
      return;
    }
    ...
    try {
      ...
      el('#apiKeyDisplay').textContent =
        ...
        data?.data?.key ||
        data?.data?.apiKey ||
        'Key created — copy it from the server response.';

      showToast('New API key generated.');
    } catch (error) {
      showToast(
        `Could not generate a key: ${error.message}`
      );
    }
  }
);

el('#revokeKey').addEventListener(
  'click',
  () => {
    if (
      confirm(
      ...
      showToast(
        'Key revoked in this dashboard. Connect your API to revoke it server-side.'
      );
    }
  }
);

const hasApi = Boolean(API.baseUrl);

el('#connectionStatus').innerHTML =
  `<span></span> ${
    hasApi ? 'API connected' : 'Demo mode'
  }`;
