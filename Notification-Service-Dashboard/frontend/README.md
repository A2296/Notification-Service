...
├── frontend/
│   ├── app.js                  Adds functionality and interactivity: switching pages, submitting notifications, registering/signing in businesses, calling backend API endpoints, filtering activity, and storing connection details in the browser.
│   ├── auth.css                Adds small, focused styling for the authentication UI, including the sign-in button, sign-in/register form messages, and dialog actions.
│   ├── index.html              Defines the dashboard’s structure and content: navigation, forms, notification tables, sign-in/register dialog, API settings, and credential screens.
│   ├── style.css               Controls the main visual design: layout, colors, dashboard cards, tables, forms, buttons, responsive/mobile styling, and navigation.
│   ├── README                  Documents the frontend: its features, expected API endpoints, how to connect it to the backend, and how to run it locally.
...
| dashtech0147 | Frontend Developer  | Designed and implemented the Notification Service dashboard, including business registration/sign-in, API connection settings, API credential management, notification composer, delivery activity tracking, and responsive user interface styling.
