const API = {
  ...
  async request(path, options = {}) {
    ...
    const response = await fetch(`${this.baseUrl.replace(/\/$/, '')}${path}`, { ...options, headers });
    if (!response.ok) {
...
function notificationRow(item) {
  return `<tr><td><strong>${escapeHtml(item.subject || item.title || 'Notification')}</strong><span class="notification-id">${escapeHtml(item.id || item._id || '—')}</span></td><td><span class="channel">${item.channel === 'email' ? '✉' : item.channel === 'sms' ? '◌' : '▣'} ${escapeHtml(item.channel || 'in-app')}</span></td><td>${escapeHtml(item.recipient || item.to || '—')}</td><td><span class="status ${escapeHtml(item.status || 'pending')} ">${escapeHtml(item.status || 'pending')}</span></td><td>${escapeHtml(formatDate(item.createdAt || item.created_at))}</td></tr>`;
}
function formatDate(value) {
  if (!value) return 'Just now';
  if (typeof value === 'string' && /ago$/.test(value)) return value;
...
el('#notificationForm').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    notifications.unshift({ id: response?.id || response?._id || `ntf_${Math.random().toString(36).slice(2, 8)}`, ...payload, status: response?.status || 'pending', createdAt: new Date().toISOString() });
